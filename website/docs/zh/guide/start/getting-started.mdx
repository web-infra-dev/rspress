---
description: 通过脚手架或手动方式快速创建 Rspress 项目，配置 Git 初始化，并了解开发服务、生产构建和本地预览的基本流程。
---
...
按照提示一步步操作即可。在创建过程中，你可以输入项目名称或路径、选择是否启用 i18n、是否创建用于自定义主题的 `theme` 目录，以及是否添加可选工具或 Skills。
...
| `basic-theme` | 创建一个带有 `theme` 目录的单语言文档站点。   |
| `i18n`        | 创建一个包含英文和中文内容的多语言文档站点。  |
| `i18n-theme`  | 创建一个带有 `theme` 目录的多语言文档站点。   |
...
- [rspress-docs-generator](https://github.com/rstackjs/agent-skills#rspress-docs-generator)：帮助 AI Agent 创建和维护 Rspress 文档站点，尤其适合在 monorepo 中维护一个独立 Rspress 项目作为文档站的场景。
...
如果你选择了 `basic-theme` 或 `i18n-theme` 模板，以下 Skill 会被默认勾选：
...
```bash
npx -y create-rspress@latest --dir my-docs --template basic-theme
```

使用缩写：

```bash
npx -y create-rspress@latest -d my-docs -t basic-theme
```

指定多个 tools：

```bash
npx -y create-rspress@latest -d my-docs -t basic-theme --tools rslint,prettier
```

跳过 Git 初始化：

```bash
npx -y create-rspress@latest -d my-docs -t basic-theme --no-git
```

创建多语言模板：

```bash
npx -y create-rspress@latest -d my-docs -t i18n
```

创建带 `theme` 目录的多语言模板：

```bash
npx -y create-rspress@latest -d my-docs -t i18n-theme
```

安装可选的 Agent Skill：

```bash
npx -y create-rspress@latest -d my-docs -t basic-theme --skill rspress-docs-generator,rspress-best-practices
```
