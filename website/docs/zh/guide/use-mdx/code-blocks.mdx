---
description: 使用 Shiki 实现编译时代码高亮，支持行高亮、行号、代码换行、Diff、外部文件引用和 Twoslash 等丰富功能。
---
...
Rspress 默认使用 [Shiki](https://shiki.style) 提供语法高亮，高亮计算都在编译期完成，这意味着运行时会有更高的性能。
...
你可以使用 [Shiki transformers](#shiki-transformers) 中的 `transformerNotationHighlight` 配合 `// [!code highlight]` 注释来高亮代码行。
...
你也可以通过在配置文件中设置 [`showLineNumbers`](/api/config/config-build#markdownshowlinenumbers) 选项来全局启用行号：
...
你也可以通过在配置文件中设置 [`defaultWrapCode`](/api/config/config-build#markdowndefaultwrapcode) 选项来全局启用代码换行：
...
你可以通过 `height` 和 `fold` meta 属性控制代码块的高度行为。共有三种情况：

- **`fold`**：可折叠代码块，底部显示展开按钮。高度默认 300px。
- **`height=X`**：固定高度，显示垂直滚动条。如果同时使用 `fold`，则以指定高度折叠而非滚动。
- **无 meta**：默认完全展开。可通过 [`markdown.defaultCodeOverflow`](/api/config/config-build#markdowndefaultcodeoverflow) 全局配置默认行为。
...
Rspress 提供了 `@rspress/plugin-twoslash` 插件，它可以在 Rspress 中使用 Twoslash 功能。详见 [@rspress/plugin-twoslash 文档](../../plugin/official-plugins/twoslash.mdx)
...
当需要在运行时动态渲染代码块，比如在交互式文档中，或者需要远程拉取代码块内容。Rspress 提供了 [`CodeBlockRuntime` 组件](../../ui/components/code-block-runtime.mdx)。
