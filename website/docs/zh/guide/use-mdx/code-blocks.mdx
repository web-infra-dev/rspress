...
当代码块内容较长时，你可以使用 `fold` 属性启用展开/收起功能。代码块会以指定高度收起，底部显示展开按钮，点击可展开查看完整代码。
...
你也可以通过 `height` 属性自定义收起高度（单位为像素，默认 300）：
...
你也可以单独使用 `height` 属性（不加 `fold`）来设置固定高度并显示垂直滚动条。如需为所有代码块全局设置默认的高度行为，请参阅 [`markdown.defaultCodeOverflow`](/api/config/config-build#markdowndefaultcodeoverview)。
