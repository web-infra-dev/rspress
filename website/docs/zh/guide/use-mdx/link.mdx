---
description: 支持文件路径和 URL 两种链接形式，以及锚点链接、自定义锚点 id 和死链检查功能。
---
...
当使用绝对路径时，根路径为 [`docs`](/api/config/config-basic#root) 目录。如果项目中使用了 [国际化](/guide/basic/i18n) 或 [多版本](/guide/basic/multi-version)，[`markdown.link.autoPrefix`](/api/config/config-build#markdownlinkautoprefix) 配置项会自动添加前缀，所以在链接中可以省略语言目录。
...
1. IDE 支持，提供代码提示、跳转到对应文件、移动文件时自动更新链接等功能。
...
[跳转到 快速开始#创建 Rspress 项目](../start/getting-started.mdx#创建-rspress-项目)
```
...
Rspress 也可以检查站内链接中的锚点是否存在于目标页面中。它会检查当前页面锚点、相对路径链接和绝对路径链接，但会跳过外部 URL 中的锚点。

:::info
锚点检查当前默认关闭，未来版本会默认开启。
:::

通过 [markdown.link.checkAnchors](/api/config/config-build#markdownlinkcheckanchors) 配置即可自动检查无效锚点。
...
类似于死链检查，Rspress 也可以检查文档中的失效图片引用。这能够发现指向不存在本地文件的图片，包括相对路径图片和引用 `public` 目录的绝对路径图片。

通过 [markdown.image.checkDeadImages](/api/config/config-build#markdownimagecheckdeadimages) 配置即可自动检查无效图片。
