...
**原因**：这是 SPA（单页应用）部署的常见问题。Rspress 构建的站点本质上是一个 SPA，客户端路由（如 `/guide/getting-started`）在初次加载后由 JavaScript 处理。当你刷新页面或直接访问某个 URL 时，请求会发送到服务器，但服务器上可能并没有对应路径的文件（尤其是在某些静态托管服务中）。

**解决方案**：配置服务器将所有未匹配的请求重定向到 `404.html`。Rspress 生成的 `404.html` 包含完整的 SPA 应用代码，能够在客户端正确处理路由并显示对应页面。
...
**Netlify**（`_redirects` 文件或 `netlify.toml`）
...
GitHub Pages 默认会使用 `404.html` 来处理所有未匹配的路由，无需额外配置。
...
:::tip 提示
如果你使用的托管服务支持 SPA 模式或「单页应用」选项，启用该选项通常会自动处理这个问题。
:::
