---
description: SSG 在构建阶段预渲染 HTML 文件，实现更快的首屏渲染和 SEO 优化，了解 SSG 与 CSR 的区别及常见问题解决方案。
---
...
SSG（Static Site Generation，静态站点生成）是指在**构建阶段**将页面预渲染为 HTML 等静态文件。
...
其中 `404.html` 是 Rspress 自动生成的 404 页面，用于处理不存在的路由。该文件在 SPA 部署时有重要作用，详见[「刷新页面时出现 404」](#refresh-404)。
...
如果你需要向 HTML 中注入自定义标签（如 meta 标签、统计代码、脚本或样式），请参考 [自定义 Head 标签](/guide/advanced/custom-head)。
