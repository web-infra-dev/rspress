---
description: 配置静态资源前缀和基础路径，将 Rspress 站点部署到 GitHub Pages、Netlify、Vercel、Zeabur 等平台。
---
...
而所谓的静态资源，指的是产物目录中的 `static` 目录，里面包含了项目所需的 JavaScript、CSS、图片等静态资源。在部署时，如果你想将这些静态资源放到 CDN 上以保证更好的访问性能，而不是像 HTML 一样直接放到服务器上，那么你需要配置静态资源的前缀，这样才能让项目正确地引用到这些静态资源，可以通过 `builderConfig.output.assetPrefix` 来完成：
...
更多说明可参考 [`base` 配置文档](/zh/api/config/config-basic#base)。
...
首先，你需要在项目根目录下创建一个名为 `.github/workflows/deploy.yml` 的文件，内容如下：
  ...
  build:
    ...
    steps:
      ...
      - name: Build with Rspress
        run: |
          pnpm rspress build --base /<repository>/
