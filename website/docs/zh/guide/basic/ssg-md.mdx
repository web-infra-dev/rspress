...
如果由于代码不兼容 SSR，无法开启 SSG 和 SSG-MD 功能，请使用 [@rspress/plugin-llms](/plugin/official-plugins/llms)。

:::

## UI 展示

当启用 `llms: true` 时，会自动在所有 H1 标题下方显示 `LlmsCopyButton` 和 `LlmsViewOptions` 组件。这使用户可以轻松复制 markdown 内容或在 ChatGPT、Claude 等 AI 工具中打开。

如果需要自定义展示，可以通过 `themeConfig.llmsUI` 进行配置：
...
更多关于 LLMS UI 配置的信息，请参阅 [themeConfig.llmsUI](/api/config/config-theme#llmsui)。
