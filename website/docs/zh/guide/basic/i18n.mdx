---
description: 配置 locales 和 lang 实现多语言文档，通过 i18n.json 定义翻译文本，在组件中使用 useI18n 获取国际化内容。
---
...
1. 定义 I18n 文本数据。
1. 配置语言列表，`rspress.config.ts` 中的 `locales`。
1. 配置默认语言，`rspress.config.ts` 中的 `lang`。
1. 新建不同的语言版本的文档。
1. 配置侧边栏和导航栏。
1. 自定义组件中使用 `useI18n`。
...
在 MDX 开发或者[自定义主题](/guide/basic/custom-theme)开发的过程中，你可能会写一些自定义组件，这些组件中也需要使用到国际化文本，那么如何获取呢？
...
在生产构建期间，Rspress 会为同一路由下实际存在的翻译页面自动添加 `<link rel="alternate" hreflang="...">` 标签。
