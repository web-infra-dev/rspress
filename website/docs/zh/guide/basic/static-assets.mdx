...
你可以在 `markdown`（或 `mdx`）文件中导入静态资源，Rspress 底层基于 [Rsbuild 静态资源处理](https://rsbuild.rs/zh/guide/basic/static-assets) 实现。相对路径和绝对路径都是支持的。
...
你也可以使用绝对路径导入静态资源。这样，Rspress 将会在`文档根目录`下的 `public` 文件夹中寻找资源。
