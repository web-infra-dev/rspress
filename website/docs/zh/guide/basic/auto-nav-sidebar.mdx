...
在 Rspress 中，除了在配置文件中通过 `themeConfig` 声明 [nav](/api/config/config-theme.html#nav) 和 [sidebar](/api/config/config-theme.html#sidebar)，你也可以通过声明 `_nav.json` 和 `_meta.json` 描述文件来自动生成导航栏和侧边栏。我们更推荐后者，它能让配置文件更简洁，同时仍包含 `themeConfig` 下的全部能力。
...
为了更好地编辑 `_nav.json` 和 `_meta.json` 文件，Rspress V2 提供了 `@rspress/core/meta-json-schema.json` 和 `@rspress/core/nav-json-schema.json` 两个 schema 文件用于 IDE 的类型提示。
  ...
  "json.schemas": [
    {
      ...
      // 或者 "url": "https://unpkg.com/@rspress/core@2.0.0-rc.2/meta-json-schema.json"
    },
    {
      "fileMatch": ["**/_nav.json"],
      "url": "./node_modules/@rspress/core/nav-json-schema.json"
      // 或者 "url": "https://unpkg.com/@rspress/core@2.0.0-rc.2/nav-json-schema.json"
    }
  ]
  // ...
}
...
其中 `_meta.json` 的内容是 `["getting-started"]`（不需要包含 `index`）。这样，当你点击 `Basic` 目录时，会显示 `index.mdx` 文件的内容。
