...
默认情况下（根目录仅存在 `_nav.json`），Rspress 会为每个子目录生成**独立的侧边栏**，侧边栏会根据当前激活的导航项自动切换。例如，点击「Guides」导航项会显示 Guides 的侧边栏，点击「Reference」则显示 Reference 的侧边栏：

```tree
docs
├── _nav.json
├── guides
│   ├── _meta.json  // /guides 的侧边栏
│   └── ...
└── reference
    ├── _meta.json  // /reference 的侧边栏
    └── ...
```
...
详细用法请参考 [Tag 组件](/ui/layout-components/tag)。
