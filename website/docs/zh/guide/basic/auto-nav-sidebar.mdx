...
在 Rspress 中，除了在配置文件中通过 `themeConfig` 声明 [nav](/api/config/config-theme.html#nav) 和 [sidebar](/api/config/config-theme.html#sidebar)，你也可以通过声明 `_nav.json` 和 `_meta.json` 描述文件来自动生成导航栏和侧边栏。我们更推荐后者，它能让配置文件更简洁，支持 HMR，同时仍包含 `themeConfig` 下的全部能力。
...
你可以通过 `icon` 配置在侧边栏标题前添加图标。它支持 emoji、HTML 或 SVG 字符串、`public` 目录资源的绝对路径、外链和 data URL。已有的 `tag` 配置仍显示在标题之后。
...
对于自动生成的文件项，也可以在页面 frontmatter 中设置 `icon`，frontmatter 的值优先于 `_meta.json`。
本地图片资源应放在 `public` 目录中，并通过 `/icon.png` 这样的绝对路径引用。

关于 `tag` 的详细用法请参考 [Tag 组件](/ui/layout-components/tag)。
