...
<Prompt
  title="自定义 Rspress 主题"
  description="复制这段 Prompt 发送给你的 AI Agent，让它帮你自定义 Rspress 主题。"
  prompt={`阅读 Rspress 自定义主题文档 https://rspress.rs/guide/basic/custom-theme.md。

1. 了解定制选项：CSS 变量、BEM 类名、wrap 模式和 eject 模式。
2. 如有需要，安装 rspress-custom-theme skill。
3. 根据我的需求，选择合适的定制方式。
4. 按照 Rspress 最佳实践实现主题变更。`}
   />

5. 对于 CSS，Rspress 提供了 [CSS 变量](#css-variables) 和 [BEM 类名](#bem-classname)进行定制。

6. 对于 JS / React，Rspress 基于 ESM 重导出覆盖实现了一套运行时接口，支持对内置组件进行修改或替换，实现自己的首页、侧边栏、搜索组件等。
