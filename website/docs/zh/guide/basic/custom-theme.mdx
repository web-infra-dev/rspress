...
## `theme/index.tsx`：利用 ESM 重导出覆盖内置组件 \{#reexport}
...
1. 执行 CLI [`rspress eject [component]`](../../api/commands.mdx#rspress-eject)，Rspress 会将指定组件的源码 eject 到本地 `theme/components` 目录，不会 eject 依赖。
...
Rspress 的组件经过细粒度的拆分，你可在 [内置组件](/ui/components/) 中看到哪些组件适合 eject。
