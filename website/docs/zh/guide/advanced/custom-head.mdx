...
## Head 组件和 useHead hook

如果你需要更复杂的 Head 定制，Rspress 同时提供了 [Head 组件](/ui/runtime-components/head) 和 [`useHead`](/ui/hooks/use-head) hook，它们都基于 unhead。

如果你想直接在 MDX 或 JSX 中编写标签，可以使用 `Head`：
...
如果你更偏向使用对象形式声明，或者最终标签需要依赖 props、状态或复用工具函数来生成，可以使用 `useHead`：
...
如果需要根据页面路由动态调整 head 内容，请使用 [全局 head 配置](#全局-head-配置) 或 [Head 组件和 useHead hook](#head-组件和-usehead-hook)。
