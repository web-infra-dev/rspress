...
| `react-router`     | `^8.0.0`               | 8        | 内置默认版本                    |
| `react-router-dom` | `^7.0.0`               | -        | 可选的项目级覆盖版本            |
| `unified`          | `^11.0.0`              | 11       | 自定义 remark/rehype 插件需兼容 |

:::tip

React Router 8 要求 Node.js 22.22+ 和 React 19.2.7+。如需使用 React Router 7，请在项目中安装 `react-router-dom@7`。Rspress 会优先使用项目安装的 `react-router-dom`，而不是内置的 React Router 8。
...
更多迁移细节请参考 [plugin-preview 迁移指南](/plugin/official-plugins/preview#从-v1-迁移)。
