...
然后通过 [自定义主题](/guide/basic/custom-theme) 将 `Search` 组件覆盖为支持 algolia 的搜索框。
...
同时启用 `@rspress/plugin-webmcp` 时，此 `Search` 组件会自动为 `rspress_search_docs` 提供 Algolia 搜索结果，无需额外配置 WebMCP。
...
通过 [Runtime API](/ui/hooks/) 组合 `docSearchProps` 可以实现搜索结果的国际化。
