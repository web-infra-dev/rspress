...
该插件内置了多种预览模式，通过调整 `preview="..."` meta 信息可切换为不同的预览模式。 ` ```tsx preview` 实际上等同于 ` ```tsx preview="internal"`，由 [defaultPreviewMode](#defaultpreviewmode) 配置。

比如你可以通过 ` ```tsx preview="iframe-follow" ` 切换为 [iframe-follow](#previewiframe-follow) 模式，

:::tip 提示

使用带有 iframe 的预览模式进行组件展示，可以隔离文档和组件的运行环境，避免样式冲突等问题。

:::

### `preview="internal"`

下面是一个 `"internal"` 默认预览模式的示例：
...
该预览模式将在代码块右侧显示一个 iframe 预览环境，下面是一个 `"iframe-follow"` 的使用示例：
...
该预览模式的特点是会显示一个固定的 iframe 在右侧，适合移动端组件库。

下面是一个 `"iframe-fixed"` 的使用示例：
...
- ` ```tsx preview="internal"`: 预览组件将使用内嵌模式渲染。
- ` ```tsx preview="iframe-follow"`: 预览组件将使用跟随模式渲染。
- ` ```tsx preview="iframe-fixed"`: 预览组件将使用固定模式渲染。
