...
- **使用 Tailwind CSS**：因为 iframe 预览环境是独立的 Rsbuild 实例，所以如果你预览的组件依赖 Tailwind CSS，需要通过 `preEntry` 注入 `@import "tailwindcss";`。
...
- 不再内置 `@rsbuild/plugin-less` 和 `@rsbuild/plugin-sass`，如需在预览中使用 Less 或 Sass，请先安装对应的 Rsbuild 插件，然后通过 `iframeOptions.builderConfig` 进行配置：
