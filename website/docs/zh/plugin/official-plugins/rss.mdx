...
- **类型**： `string`
- **默认值**：[`url`](/zh/api/config/config-basic#url) 与 [`base`](/zh/api/config/config-basic#base)

当前文档站点的站点 URL。将在 RSS 文件中使用。

当存在 `base` 配置时，`siteUrl` 需要包含 `base` 的路径。如果已经配置了 [`url`](/zh/api/config/config-basic#url) 和 [`base`](/zh/api/config/config-basic#base)，可以省略 `siteUrl`：
...
- **类型**：`RegExp | string | (RegExp | string)[] | ((item: PageIndexInfo) => boolean)`
- **必填**
...
- **默认值**：<SourceCode href="https://github.com/web-infra-dev/rspress/blob/main/packages/plugin-rss/src/createFeed.ts" />
