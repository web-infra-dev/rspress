...
:::warning 仅生产环境
此插件在开发环境中始终不生效，仅在生产环境中生效，因为它作用于构建产物。
:::

:::warning
使用此插件前，请先确保部署环境已正确配置 fallback 页面，详见 [刷新 404（fallback 配置）](/zh/guide/basic/ssg#refresh-404)。

如果你的部署平台支持配置服务端重定向（301/302），建议优先使用服务端方案，以获得更好的 SEO 和用户体验。
:::
