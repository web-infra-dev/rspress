...
- `rspress_search_docs`：通过当前启用的 Rspress 搜索服务查询文档。支持本地搜索和 [`@rspress/plugin-algolia`](/zh/plugin/official-plugins/algolia)；仅在没有可用搜索服务时不注册。
...
搜索集成可从 `@rspress/core/theme` 调用 `registerSearchProvider`。最后挂载的搜索服务会生效，并在卸载时恢复上一个服务。主题和搜索插件因此可接入稳定的 `rspress_search_docs` 工具，而无需依赖 WebMCP 内部实现。
