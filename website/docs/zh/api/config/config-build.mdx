...
- **类型**: `{ height?: number; overview?: 'fold' | 'scroll' }`
...
- `height`：高度阈值，单位为像素。未设置时不应用任何预览行为。
- `overview`：超出高度时的处理方式，默认为 `'scroll'`。
  - `'scroll'`：固定高度，显示垂直滚动条。
  - `'fold'`：可折叠，显示展开/收起按钮。
...
单个代码块可通过 `height` 或 `fold` meta 属性覆盖默认行为。详见[代码块高度](/guide/use-mdx/code-blocks#代码块高度)。
...
是否自动从 Markdown 内容中提取描述。启用后，Rspress 会提取 `h1` 标题下方第一段有内容的段落作为页面描述。如果在 [frontmatter](/api/config/config-frontmatter#description) 中指定了 `description`，则优先使用，跳过自动提取。

提取的描述用于 `<meta name="description">` 和 Open Graph 的 `<meta property="og:description">` 标签。详情请参阅[自定义 Head 标签 - description 的获取方式](/guide/advanced/custom-head#description-的获取方式)。
