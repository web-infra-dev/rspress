...
LLMS UI 组件配置。当 `enableOnH1` 为 `true` 时，会自动在所有 H1 标题下方添加 `LlmsCopyButton` 和 `LlmsViewOptions` 组件。

这在使用 [`@rspress/plugin-llms`](/plugin/official-plugins/llms) 插件生成 llms.txt 文件时非常有用，允许用户轻松复制或在 AI 工具中打开 markdown 内容。
...
是否在 H1 标题下方显示 LLMS UI 组件（复制按钮和查看选项下拉菜单）。
