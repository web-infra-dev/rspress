...
- **类型**： `Record<string, Record<string, string>> | Promise<(Record<string, Record<string, string>>) => Record<string, Record<string, string>>>`
- **默认值**： `{}`

用于自定义国际化文案数据。你可以通过这个配置项来修改 Rspress 内置的国际化文案或扩展自定义组件的文案。当扩展文案时，通常配合 [useI18n](/zh/ui/hooks/use-i18n.html) 一起使用。
