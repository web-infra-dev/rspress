...
站点的部署 URL，例如 `https://foo.github.io`。

Rspress 在生成绝对 URL 时会结合该值和 [`base`](#base)，例如 `llms.txt` 链接或插件输出。如果你的站点部署到 `https://foo.github.io/bar/`，请将 `url` 设置为 `"https://foo.github.io"`，并将 `base` 设置为 `"/bar/"`：
