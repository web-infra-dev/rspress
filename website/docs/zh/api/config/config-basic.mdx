...
开启 [`route.cleanUrls`](#routecleanurls) 后，该选项会在客户端通过 History
API 将非规范 URL 重定向到匹配路由的简洁 URL。普通页面路由不保留 trailing slash，
目录首页路由则会保留：

- `/file.html`、`/file/` 和 `/file/index.html` 会变为 `/file`。
- 如果匹配的路由由 `folder/index.md` 生成，`/folder`、`/folder.html` 和
  `/folder/index.html` 会变为 `/folder/`。
- `/zh/guide/start/introduction.html` 和
  `/zh/guide/start/introduction/index.html` 会变为
  `/zh/guide/start/introduction`，并保留实际匹配路由中的语言前缀。

页面不会重新加载，并且会保留 query 和 hash。客户端重定向执行前，服务器仍需能够返回
所请求的页面变体。该行为遵循 Cloudflare
[`auto-trailing-slash`](https://developers.cloudflare.com/workers/static-assets/routing/advanced/html-handling/#automatic-trailing-slashes-default)
HTML handling 的 canonical URL 模式。

| `cleanUrls`       | `cleanUrlsRedirect` | 生成的链接        | 客户端 URL 规范化                |
| ----------------- | ------------------- | ----------------- | -------------------------------- |
| `false`（默认值） | `true`（默认值）    | 带 `.html` 的 URL | 不启用                           |
| `false`           | `false`             | 带 `.html` 的 URL | 不启用                           |
| `true`            | `true`（默认值）    | 简洁 URL          | 根据路由的 canonical path 规范化 |
| `true`            | `false`             | 简洁 URL          | 不启用                           |

如果只需要生成简洁链接，不希望规范化浏览器 URL，可以将 `cleanUrlsRedirect` 设置为
`false`：
