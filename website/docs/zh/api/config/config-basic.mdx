...
`localeRedirect` 已从 `themeConfig` 移至 `route`。原有的 `themeConfig.localeRedirect` 仍会继续生效以保持向后兼容，但已废弃。请迁移至 `route.localeRedirect`；同时配置两者时，`route.localeRedirect` 优先生效。

自动语言重定向现在也会在 hydration 前执行，因此首次访问的用户可能会在任何主题或客户端代码运行前被重定向。如果服务器或 CDN 已经处理了语言协商，或者站点必须保留用户最初请求的语言，请将 `route.localeRedirect` 设置为 `'never'`。
