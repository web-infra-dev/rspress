...
如果用户访问缺少末尾 `/` 的 `https://example.com/docs`，并且服务器仍然返回了 Rspress 首页，Rspress 会在浏览器中自动跳转到 `https://example.com/docs/`。查询参数和 hash 会被保留。

该选项默认开启，专门作为兜底。如果服务器直接返回 `404`，该选项无法生效。如果服务器已经会将 `/docs` 重定向到 `/docs/`，可以将 `baseRedirect` 设置为 `false`。
