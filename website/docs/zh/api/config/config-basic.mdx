...
是否开启 [SSG-MD](/guide/basic/ssg-md) 来生成 `llms.txt`、`llms-full.txt` 以及每个页面对应的 Markdown 文件，使你的文档更容易被大语言模型理解。

可通过 `llmsTxt` 基于站点信息和页面分组编排完整的 `llms.txt` 内容。详见[自定义 llms.txt](/guide/basic/ssg-md#自定义-llmstxt)。
...
`llms` 为实验性功能。如果由于代码不兼容 SSR 而无法开启 SSG-MD，请使用 [@rspress/plugin-llms](/plugin/official-plugins/llms) 作为备选方案。

:::

详细用法、配置选项和实现原理请参阅 [llms.txt（SSG-MD）](/guide/basic/ssg-md)。
...
配置 [`multiVersion`](/zh/guide/basic/multi-version) 后，默认会为每个版本创建独立的搜索索引，使搜索结果仅包含用户当前所选版本的页面。将 `versioned` 设为 `false` 可以关闭该行为，搜索结果将包含所有版本的文档。
...
开启后，该选项会以实际匹配到的路由为准，通过 History API 将非规范 URL 重定向到
[`route.cleanUrls`](#routecleanurls) 选择的 URL 格式。页面不会重新加载，并且会保留
base、语言前缀、query、hash 和 history state。

#### `cleanUrls: true`

普通页面路由不保留 trailing slash，目录首页路由则会保留。

| 传入 URL             | 匹配路由   | 客户端行为                   | 最终 URL   |
| -------------------- | ---------- | ---------------------------- | ---------- |
| `/file`              | `/file`    | 不重定向                     | `/file`    |
| `/file.html`         | `/file`    | `replaceState` 到 `/file`    | `/file`    |
| `/file/`             | `/file`    | `replaceState` 到 `/file`    | `/file`    |
| `/file/index`        | `/file`    | `replaceState` 到 `/file`    | `/file`    |
| `/file/index.html`   | `/file`    | `replaceState` 到 `/file`    | `/file`    |
| `/folder`            | `/folder/` | `replaceState` 到 `/folder/` | `/folder/` |
| `/folder.html`       | `/folder/` | `replaceState` 到 `/folder/` | `/folder/` |
| `/folder/`           | `/folder/` | 不重定向                     | `/folder/` |
| `/folder/index`      | `/folder/` | `replaceState` 到 `/folder/` | `/folder/` |
| `/folder/index.html` | `/folder/` | `replaceState` 到 `/folder/` | `/folder/` |
...
| 传入 URL             | 匹配路由   | 客户端行为                             | 最终 URL             |
| -------------------- | ---------- | -------------------------------------- | -------------------- |
| `/file`              | `/file`    | `replaceState` 到 `/file.html`         | `/file.html`         |
| `/file.html`         | `/file`    | 不重定向                               | `/file.html`         |
| `/file/`             | `/file`    | `replaceState` 到 `/file.html`         | `/file.html`         |
| `/file/index`        | `/file`    | `replaceState` 到 `/file.html`         | `/file.html`         |
| `/file/index.html`   | `/file`    | `replaceState` 到 `/file.html`         | `/file.html`         |
| `/folder`            | `/folder/` | `replaceState` 到 `/folder/index.html` | `/folder/index.html` |
| `/folder.html`       | `/folder/` | `replaceState` 到 `/folder/index.html` | `/folder/index.html` |
| `/folder/`           | `/folder/` | `replaceState` 到 `/folder/index.html` | `/folder/index.html` |
| `/folder/index`      | `/folder/` | `replaceState` 到 `/folder/index.html` | `/folder/index.html` |
| `/folder/index.html` | `/folder/` | 不重定向                               | `/folder/index.html` |
...
客户端重定向执行前，服务器仍需能够返回所请求的页面变体。该行为遵循 Cloudflare
