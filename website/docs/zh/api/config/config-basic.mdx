...
Rspress 在生成需要绝对 URL 的文件时会结合该值和 [`base`](#base)，例如 `sitemap.xml`、`llms.txt` 链接或插件输出。完整 URL 拼接顺序是 `siteOrigin + base + routePath`。
...
- **类型**：`boolean | SitemapOptions`
- **默认值**：`true`

在生产构建时向输出目录生成 `sitemap.xml`。此功能已内置于 Rspress，无需安装插件，在 `ssg: false` 时也可使用。

配置 [`siteOrigin`](#siteorigin) 可让 sitemap 包含绝对 URL。Rspress 会将 `siteOrigin`、[`base`](#base) 和每个页面的路由路径拼接起来：
...
设置 `sitemap: false` 可关闭生成，也可以传入选项对象进行自定义：
...
选项类型如下，可从 `@rspress/core` 导入 `SitemapOptions`：
...
- **类型**：`string`
- **默认值**：[`siteOrigin`](#siteorigin) + [`base`](#base)，未配置 `siteOrigin` 时使用 `base`

覆盖 sitemap 条目使用的部署 URL。请使用包含基础路径的绝对 URL，例如 `https://example.com/docs/`。此值优先于 `siteOrigin` 和 `base`。

如果既没有配置 `sitemap.siteUrl`，也没有配置 `siteOrigin`，条目会使用 `base + routePath`，保留为相对路径。配置其中任意一项即可生成绝对 sitemap URL。

### sitemap.customMaps

- **类型**：`Record<string, Sitemap>`
- **默认值**：`{}`

覆盖指定页面的 sitemap 值。键为不含 `base` 的路由路径，值会覆盖生成的 `loc`、`lastmod`、`changefreq` 和 `priority`。`lastmod` 默认使用源文件的修改时间。此选项用于自定义已有页面，不会添加路由。
...
每个页面默认的 `changefreq` 值。可选值为 `'always'`、`'hourly'`、`'daily'`、`'weekly'`、`'monthly'`、`'yearly'` 和 `'never'`。可通过 `customMaps` 为指定页面覆盖此值。
...
每个页面默认的 `priority` 值，可选值从 `'0.0'` 到 `'1.0'`，间隔为 `0.1`。根路由 `/` 使用 `'1.0'`。可通过 `customMaps` 为指定页面覆盖此值。
