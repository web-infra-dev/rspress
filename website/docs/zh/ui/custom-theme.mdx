...
一方面你需要通过 `export` 导出一个主题配置对象，另一方面你需要通过 `export * from '@rspress/core/theme-original'` 导出所有具名导出的内容，这样才能保证你的主题配置能够正常工作。
...
当然，在开发过程可能需要使用页面的数据，你可以通过 [`Runtime API`](/ui/hooks/) 来获取。
