...
Rspress 是一个基于 Rsbuild 的静态站点生成器，致力于让每个文档都能被 Agent 更好地发现、读取和理解。本文将介绍 Rspress 在 Agent-friendly 上的最佳实践。
...
[llms.txt 规范](https://llmstxt.org/)定义了一种放在网站根目录或子路径下的 Markdown 索引文件，包含网站简介和指向详细内容的链接。

`sitemap.xml` 面向搜索引擎收录页面；`llms.txt` 面向 Agent 的内容发现和渐进式披露：先提供精简的文档索引，再让 Agent 按需读取相关页面。

例如 Rspress 站点的 `llms.txt` 文件位于 [https://rspress.rs/llms.txt](https://rspress.rs/llms.txt)：
...
├── sitemap.xml                             # HTML 文档索引
├── llms.txt                                # Markdown 精简的索引文件，链接到页面内容
...
Rspress 默认启用 [`injectLlmsHint`](/zh/api/config/config-theme#injectllmshint) 配置。在启用 SSG-MD 的同时，生成的 HTML 会在正文靠前的位置注入一段视觉隐藏的纯文本：
...
两种输出分别对应 `llms-txt-directive-html` 和 `llms-txt-directive-md`。
