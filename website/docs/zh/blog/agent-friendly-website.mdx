...
![Rspress 的 AFDocs 评分](./agent-friendly-website-afdocs-scorecard.png)

_Rspress 在 afdocs 中的评分为 100/100_
...
与 Agent-friendly 相关的 `doc_build` 产物可以分为三部分：给人阅读的 HTML、给 Agent 逐页读取的 Markdown，以及用于内容发现的索引文件。
...
值得注意的是，AFDocs 不评价文章质量或 Agent 回答的正确率，只检查可验证的访问行为：`/llms.txt` 是否存在、`.md` URL 是否可访问、`Accept: text/markdown` 是否返回 Markdown，以及 HTML 与 Markdown 是否内容一致。
...
Agent 可能从深层文档进入网站，不经过首页或 `/llms.txt`。因此，每个页面都需要暴露 `llms.txt` 地址。
