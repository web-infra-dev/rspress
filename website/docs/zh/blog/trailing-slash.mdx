...
客户端路由通常采用比较宽松的匹配策略。例如，[React Router 默认会忽略 path 末尾的 `/`](https://github.com/remix-run/react-router/blob/react-router%407.18.2/packages/react-router/lib/router/utils.ts#L1576-L1578)，因此下面两个 URL 都能匹配 path 为 `/guide` 的同一个路由：

- `/guide`
- `/guide/`

React Router 不会自动移除 `.html`，也不会将 `/index.html` 视为目录首页。`/guide.html` 和 `/guide/index.html` 是否可以访问，取决于框架或 Web server 的额外处理。
