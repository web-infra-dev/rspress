...
This preview mode will display an iframe preview environment on the right side of the code block. Here is an example of `"iframe-follow"`:
...
This preview mode features a fixed iframe on the right side, suitable for mobile component libraries.

Here is an example of `"iframe-fixed"`:
