...
- `@rsbuild/plugin-less` and `@rsbuild/plugin-sass` are no longer built-in. If you need Less or Sass support in previews, manually configure the corresponding plugin via `iframeOptions.builderConfig`:
