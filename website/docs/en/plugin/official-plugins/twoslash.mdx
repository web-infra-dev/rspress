...
`cache` is used to cache the TypeScript language servers based on compiler options when [createTwoslasher](https://twoslash.netlify.app/refs/api#createtwoslasher). Default is `true`.
