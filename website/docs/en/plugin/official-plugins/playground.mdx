...
Use the ` ```jsx playground ` syntax in mdx files:
...
3. It is recommended to use jsx to write example code to avoid ts type errors in the editor.
...
- ` ```jsx pure`: Render as a regular code block
- ` ```jsx `: Render based on `defaultRenderMode` configuration
- ` ```jsx playground`: Render as an editable Playground component
