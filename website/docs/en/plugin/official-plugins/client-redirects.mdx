...
:::warning
Before using this plugin, make sure your deployment environment has the fallback page correctly configured. See [FAQ](/guide/basic/ssg#refresh-404) for details.

Note: Client-side redirects differ from server-side redirects — the page will load first and then redirect (causing a brief flash), and SSR is not supported.
:::
