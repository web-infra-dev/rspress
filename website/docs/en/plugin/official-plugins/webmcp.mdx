...
Search integrations can call `registerSearchProvider` from `@rspress/core/theme`. The most recently mounted provider is used, and unmounting it restores the previous provider. This lets theme and search plugins integrate with the stable `rspress_search_docs` tool without depending on WebMCP internals.
