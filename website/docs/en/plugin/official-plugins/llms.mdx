...
This plugin is only intended as a fallback solution when SSG and SSG-MD features cannot be enabled due to code incompatibility with SSR. It is recommended to prioritize using the [SSG-MD](/guide/basic/ssg-md) feature.
...
If you want users reading your documentation site to better utilize large language models to read the documentation, you can add a copy Markdown button at the top of the page with the same effect as this website.

#### Using themeConfig (Recommended)

The simplest way is to enable `llmsUI.enableOnH1` in `themeConfig`. This will automatically add `LlmsCopyButton` and `LlmsViewOptions` components below all H1 headers without requiring custom theme code:
...
export default defineConfig({
  ...
  themeConfig: {
    llmsUI: {
      enableOnH1: true,
      // Optional: customize view options
