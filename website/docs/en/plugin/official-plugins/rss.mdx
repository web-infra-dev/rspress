...
- **Type**: `string`
- **Default**: [`url`](/api/config/config-basic#url) with [`base`](/api/config/config-basic#base)

The site URL of the current document site. It will be used in the RSS file.

When there is a `base` configuration, `siteUrl` needs to include the `base` path. If [`url`](/api/config/config-basic#url) and [`base`](/api/config/config-basic#base) are configured, you can omit `siteUrl`:
