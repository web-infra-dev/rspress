...
If there is a misjudgment of anchors, you can ignore the error through the `excludes` configuration:
...
After enabling this configuration, Rspress will check the local images in the document. If an image references a non-existent file, the build will throw an error and exit.
...
If there is a misjudgment of images, you can ignore the error through the `excludes` configuration:
...
Individual code blocks can override the default using `height` or `fold` meta attributes. See [Code block height](/guide/use-mdx/code-blocks#code-block-height) for details.
...
Whether to automatically extract the description from the markdown content. When enabled, Rspress extracts the first contentful paragraph below the `h1` heading as the page description. If `description` is specified in [frontmatter](/api/config/config-frontmatter#description), it takes priority and automatic extraction is skipped.

The extracted description is used for `<meta name="description">` and Open Graph `<meta property="og:description">` tags. See [Custom Head - How description is determined](/guide/advanced/custom-head#how-description-is-determined) for details.
