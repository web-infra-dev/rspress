...
### markdown.defaultCodeOverview

- **Type**: `{ height?: number; overview?: 'fold' | 'scroll' }`
...
- `height`: Height threshold in pixels. When not set, no overview behavior is applied.
- `overview`: How to handle code blocks exceeding the height. Defaults to `'scroll'`.
