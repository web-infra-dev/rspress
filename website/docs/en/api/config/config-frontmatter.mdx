...
By default, Rspress extracts the first contentful paragraph below the `h1` heading as the description (see [`markdown.extractDescription`](/api/config/config-build#markdownextractdescription)). If the extracted result does not meet your needs, you can use this field to override it:
