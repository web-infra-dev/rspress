...
Configuration for LLMS UI components. When `enableOnH1` is `true`, `LlmsCopyButton` and `LlmsViewOptions` components will be automatically added below all H1 headers.

This is useful when using the [`@rspress/plugin-llms`](/plugin/official-plugins/llms) plugin to generate llms.txt files, allowing users to easily copy or open the markdown content in AI tools.
...
export default defineConfig({
  ...
  themeConfig: {
    llmsUI: {
      enableOnH1: true,
      // Optional: customize dropdown menu options
...
Whether to show LLMS UI components (copy button and view options dropdown) below H1 headers.
