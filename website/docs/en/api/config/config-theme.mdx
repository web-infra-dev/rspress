...
- In HTML output, it renders a visually hidden plain-text DOM element near the top of the page. It does not use `display: none`, the `hidden` attribute, `aria-hidden`, or nested links, so agent-side HTML-to-Markdown conversion can preserve the directive as text.
- In SSG-MD output, it renders a blockquote string near the top of the Markdown page.
...
Set `injectLlmsHint` to `false` to remove the directive hint without disabling the visible LLMS UI components:
