...
If a user opens `https://example.com/docs` without the final `/` and the server still returns the Rspress homepage, Rspress redirects the browser to `https://example.com/docs/`. Query parameters and hashes are preserved.

This option is enabled by default as a fallback. It cannot take effect if the server returns a `404`. If your server already redirects `/docs` to `/docs/`, you can set `baseRedirect` to `false`.
