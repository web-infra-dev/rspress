...
- **Type**: `Record<string, Record<string, string>> | Promise<(Record<string, Record<string, string>>) => Record<string, Record<string, string>>>`
- **Default**: `{}`

Used to customize internationalization (i18n) text data. You can use this configuration to modify Rspress's built-in i18n text or extend custom component text. When extending text, it is usually used together with [useI18n](/en/ui/hooks/use-i18n.html).
...
Example of modifying Rspress's built-in i18n text:
...
export default defineConfig({
  i18nSource: async (source) => {
    // Modify or extend the i18n source
    source.customKey = {
      zh: '自定义文案',
      en: 'Custom Text',
    };
