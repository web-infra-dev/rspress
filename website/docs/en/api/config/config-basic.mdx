...
Enables a client-side fallback that redirects the configured base path without a trailing slash to its canonical trailing-slash URL. For example, when `base` is `/docs/`, a request to `/docs` is redirected to `/docs/`. Query parameters and hashes are preserved.

This fallback only works when the deployment server returns the Rspress homepage HTML and client assets for the URL without the trailing slash. It cannot recover from a server-side `404`.

:::tip Prefer server-side redirects

For production sites, prefer a permanent redirect on the server or at the CDN edge. It canonicalizes the URL before HTML is delivered and does not depend on client-side JavaScript. Keep `route.baseRedirect` enabled as a fallback when the hosting server may serve the homepage HTML without performing that redirect.

:::

Set this option to `false` when your server or CDN already canonicalizes the base URL, or when both URL forms must remain accessible:
