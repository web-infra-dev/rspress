...
`localeRedirect` has moved from `themeConfig` to `route`. The former `themeConfig.localeRedirect` option remains supported for backward compatibility, but is deprecated. Migrate to `route.localeRedirect`; when both options are set, `route.localeRedirect` takes precedence.

Automatic locale redirects also now run before hydration, so first-time visitors may be redirected before any theme or client code runs. Set `route.localeRedirect` to `'never'` if your server or CDN already handles locale negotiation, or if the site must preserve the originally requested locale.
