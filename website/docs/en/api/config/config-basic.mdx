...
`localeRedirect` has moved from `themeConfig` to `route`. Existing `themeConfig.localeRedirect` values are no longer read; move them to `route.localeRedirect` when upgrading.
