...
When [`route.cleanUrls`](#routecleanurls) is enabled, this option redirects
non-canonical URLs to the matching route's clean URL on the client using the
History API. Regular page routes omit the trailing slash, while directory index
routes retain it:

- `/file.html`, `/file/`, and `/file/index.html` become `/file`.
- `/folder`, `/folder.html`, and `/folder/index.html` become `/folder/` when the
  matching route is generated from `folder/index.md`.
- `/zh/guide/start/introduction.html` and
  `/zh/guide/start/introduction/index.html` become
  `/zh/guide/start/introduction`, preserving the locale prefix from the matched
  route.

The page is not reloaded, and the query string and hash are preserved.
...
Cloudflare's [`auto-trailing-slash`](https://developers.cloudflare.com/workers/static-assets/routing/advanced/html-handling/#automatic-trailing-slashes-default)
HTML handling.

| `cleanUrls`       | `cleanUrlsRedirect` | Generated links   | Client URL normalization               |
| ----------------- | ------------------- | ----------------- | -------------------------------------- |
| `false` (default) | `true` (default)    | URLs with `.html` | Disabled                               |
| `false`           | `false`             | URLs with `.html` | Disabled                               |
| `true`            | `true` (default)    | Clean URLs        | Enabled using the canonical route path |
| `true`            | `false`             | Clean URLs        | Disabled                               |

Set `cleanUrlsRedirect` to `false` to generate clean links without normalizing
the browser URL:
