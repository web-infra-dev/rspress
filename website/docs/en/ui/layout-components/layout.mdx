...
const Layout = () => (
  <BasicLayout
    ...
    beforeLeftNavItems={<div>beforeLeftNavItems</div>}
    afterLeftNavItems={<div>afterLeftNavItems</div>}
    /* Before/after the right navigation items (desktop and mobile) */
    beforeRightNavItems={<div>beforeRightNavItems</div>}
    afterRightNavItems={<div>afterRightNavItems</div>}
    /* Above left sidebar */
...
Use `beforeLeftNavItems`, `afterLeftNavItems`, `beforeRightNavItems`, and `afterRightNavItems` to insert custom navigation content. With [NavList](./nav-list), the same `items` automatically render as a horizontal desktop menu or a vertical list in the expanded mobile menu, without checking the viewport width.
