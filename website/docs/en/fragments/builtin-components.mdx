...
<PackageManagerTabs command="install -D @rspress/core @rr" />
...
In the install command, special processing has been done for yarn, pnpm, bun and deno. If your command is `install some-packages`, the install will be automatically replaced with add in the yarn/pnpm/bun/deno tab, and the `npm:` prefix will be added to the package in the deno tab.
