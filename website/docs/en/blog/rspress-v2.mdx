...
Rspress is a static site generator built on [Rsbuild](https://rsbuild.rs/), designed as a developer-oriented documentation site tool. Three years have passed since Rspress was first officially released in 2023. As of version 1.47.0, Rspress has iterated through **144 releases** and received extensive feedback and contributions from the community, with **125 contributors** participating in the Rspress project. With the community's contributions and support, more and more developers have leveraged Rspress's **blazing-fast compilation performance, convention-based routing, and component library previews** to build beautiful and reliable documentation sites.

Building on this foundation, based on user feedback and suggestions, we aim to make further progress in [**theme aesthetics**](#brand-new-theme), [**AI-native**](#llms-txt-ssg-md), [**documentation development experience**](#doc-dx), [**integration with Rslib**](#rslib-rspress), and more.
...
- **On-demand Compilation, Instant Startup**: [lazyCompilation](https://rspack.rs/guide/features/lazy-compilation) is enabled by default, combined with resource preloading on link hover, building only the required files when visiting specific routes, achieving **instant startup** regardless of project size.
...
This is a comprehensive upgrade to the existing architecture. Below we introduce Rspress 2.0 and its **brand new theme, high-quality llms.txt generation, Shiki integration, on-demand compilation**, and other important features.
...
The 2.0 default theme has undergone a systematic upgrade, designed by team designer [@Zovn Wei](https://x.com/wei_zhong41532), with significant improvements in visual effects and reading experience. Each component is individually replaceable, providing high customizability.
...
In React-based dynamic rendering frontend frameworks, there is often the problem of difficulty extracting static information, and Rspress faces the same challenge. Rspress allows users to enhance documentation expressiveness through [MDX fragments](/guide/use-mdx/components), React components, Hooks, and TSX routes. However, this dynamic content faces the following issues when converting to Markdown text:
...
To solve this problem, Rspress 2.0 introduces the [SSG-MD](/guide/basic/ssg-md) feature. This is a brand new capability, similar to [Static Site Generation (SSG)](/guide/basic/ssg), but instead of rendering pages as HTML files, it renders them as Markdown files and generates [llms.txt](https://llmstxt.org/) and llms-full.txt files.
...
Compared to traditional approaches like converting HTML to Markdown, SSG-MD has access to better information sources during rendering, such as the React virtual DOM, resulting in better static information quality and flexibility.

<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img src="./imgs/ssg-md-flow.jpg" width="600" alt="SSG-MD rendering flow" />
</div>
...
This ensures both the interactive documentation experience and helps AI understand the semantic information of components.
...
> Using [transformerNotationFocus](/guide/use-mdx/code-blocks#transformer-notation-focus)
...
Rspress 2.0 is powered by Rsbuild and Rspack 2.0-alpha, with [lazyCompilation](https://rspack.rs/config/experiments#experimentslazycompilation) and [persistent cache](https://rsbuild.rs/config/performance/build-cache) enabled by default.

#### lazyCompilation On-demand Compilation

lazyCompilation compiles on-demand in dev mode — pages are only compiled when you visit them, dramatically improving development startup speed and even achieving millisecond-level cold starts. Rspress also implements a route preload strategy that preloads target route pages when hovering over links, working together with lazyCompilation to provide a lossless development experience.

<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img
    src="./imgs/lazy-compilation.gif"
    width="600"
    alt="lazyCompilation on-demand compilation demo"
  />
</div>
...
The template includes the [rsbuild-plugin-workspace-dev](https://github.com/rstackjs/rsbuild-plugin-workspace-dev) plugin, which starts Rslib's watch command alongside the Rspress development server.
...
This doesn't mean we're abandoning Rust for the Markdown ecosystem. In the future, we may continue to advance Rust-based Markdown parser integration in Rspack, providing performance optimization at a lower level.
