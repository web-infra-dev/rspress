...
Rspress is a static site generator built on Rsbuild. It aims to make every document easier for Agents to discover, read, and understand. This article introduces Rspress's Agent-friendly best practices.
...
The [llms.txt specification](https://llmstxt.org/) defines a Markdown index file placed at the root or a subpath of a website. It contains an introduction to the website and links to detailed content.
...
For example, the Rspress website's `llms.txt` file is available at [https://rspress.rs/llms.txt](https://rspress.rs/llms.txt):
...
├── sitemap.xml                             # HTML documentation index
├── llms.txt                                # Concise Markdown index file linking to page content
...
Rspress enables [`injectLlmsHint`](/api/config/config-theme#injectllmshint) by default. When SSG-MD is enabled, Rspress injects visually hidden plain text near the beginning of the main content in generated HTML:
...
The generated Markdown output also includes the following content:
...
The two outputs correspond to `llms-txt-directive-html` and `llms-txt-directive-md`.
