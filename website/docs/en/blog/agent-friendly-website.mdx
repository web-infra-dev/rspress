...
Several Rspress Agent-friendly optimizations described in this article also correspond to AFDocs checks:
...
Notably, AFDocs does not evaluate writing quality or the correctness of Agent answers. It only checks verifiable access behavior: whether `/llms.txt` exists, whether `.md` URLs are accessible, whether `Accept: text/markdown` returns Markdown, and whether HTML and Markdown contain equivalent content.
