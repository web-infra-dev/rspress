...
| `react-router-dom` | `^7.0.0`               | -       | Optional project override                       |
| `unified`          | `^11.0.0`              | 11      | Custom remark/rehype plugins must be compatible |

:::tip

React Router 8 requires Node.js 22.22+ and React 19.2.7+. To use React Router 7 instead, install `react-router-dom@7` in your project. Rspress will prioritize the project-installed `react-router-dom` over its built-in React Router 8.
...
We recommend using a global find-and-replace to update import paths.
...
The following packages are now built into `@rspress/core`. If you are upgrading from 1.x, use the import path changes table above to update your import paths:
...
For more migration details, see [plugin-preview migration guide](/plugin/official-plugins/preview#migrating-from-v1).
