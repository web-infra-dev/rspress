...
`@rspress/plugin-preview` V2 no longer includes `@rsbuild/plugin-less` and `@rsbuild/plugin-sass` as built-in dependencies. If you need Less or Sass support in previews, manually configure the corresponding plugin via `iframeOptions.builderConfig`:
