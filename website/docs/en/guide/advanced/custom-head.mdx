...
2. **Automatic extraction** — Otherwise, Rspress automatically extracts the description from the markdown content (enabled by default via [`markdown.extractDescription`](/api/config/config-build#markdownextractdescription)). It collects all text between the `h1` heading and the first `h2` heading, while skipping code blocks, images, HTML, tables, import/export statements, and [container directives](/guide/use-mdx/container) (e.g., `:::tip ... :::`). Link URLs are stripped, keeping only the link text.
