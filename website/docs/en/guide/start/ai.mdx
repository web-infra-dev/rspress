...
When creating a new project with `create rspress@latest`, the CLI prompts you to install recommended Rspress Agent Skills. The default suggestions include `rspress-best-practices` and `rspress-description-generator`; if you choose the custom theme template, `rspress-custom-theme` is also available.
