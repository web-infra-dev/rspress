...
The default theme exposes more CSS variables for theme colors, code blocks, homepage components, and more. All built-in components also follow the [BEM naming convention](https://getbem.com/), making it easier to override styles with standard CSS selectors. If CSS is not enough, you can go deeper by overriding built-in components through ESM re-exports in `theme/index.tsx`.
...
- **Programming config**. Implement it by specifying the [nav](/api/config/config-theme#nav) and [sidebar](/api/config/config-theme#sidebar) config items in the Rspress config.
...
At the same time, we also provide config to easily customize the HTML content generated by SSG. For details, please refer to the [Static Site Generation Document](/guide/basic/ssg).
...
Rspress includes translations for Chinese, English, Japanese, Korean, and more languages built-in, with more to come. The system automatically performs "Tree Shaking" based on language configuration and usage, bundling only the text you need. You can also easily extend or override translations through the [`i18nSource`](/api/config/config-basic#i18nsource) configuration option.
...
For details about custom themes, you can refer to the [Custom Theme Documentation](/guide/basic/custom-theme).

### Plugin system

The plugin system is a crucial part of Rspress, which can help you conveniently extend Rspress's functionality during site construction. Details can be found in the [Plugin Introduction Documentation](/plugin/system/introduction).
...
After enabling `@rspress/plugin-preview`, you can declare a `preview` code block directly in `.mdx`:
...
When readers need to edit and run the example directly in the document, you can use `playground`:
