...
Build mode enables SSG by default. During the build, Rspress executes React component rendering in a Node.js environment, converting each page into an HTML file with complete content. You can control the output type through the `ssg` configuration.
