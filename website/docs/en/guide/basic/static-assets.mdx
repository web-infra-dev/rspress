...
You can import static assets in `markdown` (or `mdx`) files. Rspress is built on [Rsbuild static assets handling](https://rsbuild.rs/guide/basic/static-assets) under the hood. Both relative paths and absolute paths are supported.
...
You can also import static assets using absolute paths. In this case, Rspress will look for assets in the `public` folder under the `document root directory`.
