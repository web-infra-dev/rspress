---
description: Configure site logo, favicon icons, reference images and videos in MDX files, and use the public directory for custom static resources.
---
...
This is equivalent to using `import image from "./demo.png"` directly.

Not only images, but you can also import videos, audios and other static assets in markdown files.

### public directory

The public directory under the docs directory can be used to place static assets. These assets are not processed during the build and can be referenced directly via URL.

- When you start the dev server, these assets will be served under the [server.base](https://rsbuild.rs/config/server/base) root path (default `/`).
- When you run a production build, these assets will be copied to the [dist directory](https://rsbuild.rs/guide/basic/output-files).
...
When your site is configured with a `base` path and you want to use the `img` tag to introduce an absolute path, you need to use `normalizeImagePath` provided by `@rspress/core/runtime` to manually add the `base` path to its src. Here is an example:
