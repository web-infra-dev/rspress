...
You can also import static assets using absolute paths. In this case, Rspress will look for assets in the `public` folder under the `document root directory`. These assets are not processed during the build and are referenced directly via URL.
