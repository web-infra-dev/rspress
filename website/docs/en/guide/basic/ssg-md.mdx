...
Rspress provides experimental Static Site Generation to Markdown (SSG-MD) capability, which is a brand new feature. As its name suggests SSG-MD, the only difference from [Static Site Generation (SSG)](./ssg) is that it renders your pages as Markdown files instead of HTML files, and generates [`llms.txt`](#llms-txt) and llms-full.txt related files, making it easier for large language models to understand and use your technical documentation.

Comparison between SSG and SSG-MD:
...
| **Index File**          | `sitemap.xml`                             | `llms.txt`                                     |
| **Full Content File**   | -                                         | `llms-full.txt`                                |
| **Index Content**       | Page URLs, update time, priority          | Page titles, descriptions, in navigation order |
| **Core Implementation** | `renderToString`                          | `renderToMarkdownString`                       |
| **Standard**            | [sitemaps.org](https://www.sitemaps.org/) | [llmstxt.org](https://llmstxt.org/)            |
