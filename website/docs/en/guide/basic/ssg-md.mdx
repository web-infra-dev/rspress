---
tag: experimental
description: Generate llms.txt and Markdown files for AI-friendly documentation using SSG-MD, enabling better understanding by large language models.
---
...
When `llms: true` is enabled, `LlmsCopyButton` and `LlmsViewOptions` components will be automatically displayed below all H1 headers. This allows users to easily copy markdown content or open it in AI tools like ChatGPT or Claude.

If you want to customize the display, you can configure it through `themeConfig.llmsUI`:
...
For more information about LLMS UI configuration, see [themeConfig.llmsUI](/api/config/config-theme#llmsui).
