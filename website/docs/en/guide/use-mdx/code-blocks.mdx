...
When using external file code blocks, it's common to use them together with [route conventions](./components.mdx#routing-convention). Name files starting with `_`.
...
- **No meta**: Fully expanded by default. You can change this globally via [`markdown.defaultCodeOverflow`](/api/config/config-build#markdowndefaultcodeoverflow).
...
:::warning
Use `CodeBlockRuntime` only when necessary. It increases runtime bundle size and cannot benefit from compile-time highlighting.
:::
