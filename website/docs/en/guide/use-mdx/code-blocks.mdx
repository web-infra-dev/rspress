...
You can also use the `height` attribute alone (without `fold`) to set a fixed height with a vertical scrollbar. To apply a default height behavior to all code blocks globally, see [`markdown.defaultCodeOverflow`](/api/config/config-build#markdowndefaultcodeoverview).
