...
Use the `fold` attribute to enable expand/collapse functionality:
...
You can customize the collapsed height via the `height` attribute (in pixels, default 300):
...
````mdx
```tsx height="200"
// This code block has a fixed height of 200px with scrollbar
```
````
