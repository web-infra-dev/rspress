---
description: Use Shiki for syntax highlighting with support for line highlighting, code titles, line numbers, word highlighting, diff display, and code folding.
---
...
You can also enable line numbers globally by setting the [`showLineNumbers`](/api/config/config-build#markdownshowlinenumbers) option in the config file:
...
You can also enable code wrapping globally by setting the [`defaultWrapCode`](/api/config/config-build#markdowndefaultwrapcode) option in the config file:
...
When a code block is long, you can use the `fold` attribute to enable expand/collapse functionality. The code block will be collapsed to a specified height with an expand button at the bottom. Click to expand and view the full code.
...
You can also customize the collapsed height via the `height` attribute (in pixels, default 300):

````mdx
```tsx fold height="200"
// This code block collapses to 200px
```
````
...
You can also use the `height` attribute alone (without `fold`) to set a fixed height with a vertical scrollbar. To apply a default height behavior to all code blocks globally, see [`markdown.defaultCodeOverview`](/api/config/config-build#markdowndefaultcodeoverview).
...
Rspress uses [Shiki](https://shiki.style) for compile-time code highlighting, providing flexible code block capabilities.
...
Rspress provides the `@rspress/plugin-twoslash` plugin, which enables Twoslash features in Rspress. See [@rspress/plugin-twoslash documentation](../../plugin/official-plugins/twoslash.mdx) for details.
