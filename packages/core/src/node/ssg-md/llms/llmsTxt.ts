...
function routePathToMdPath(
  ...
  return withSiteUrl(withBase(url, base), siteUrl);
}
    ...
    const routeLines: string[] = (
      await Promise.all(
        routes.map(async route => {
          ...
          return `- [${title}](${routePathToMdPath(routePath, base, siteUrl)})${description ? `: ${description}` : ''}`;
        }),
      )
...
function generateLlmsFullTxt(
  ...
  for (let i = 0; i < navList.length; i++) {
    ...
    for (const routePath of routeGroup) {
      lines.push(`---
url: ${routePathToMdPath(routePath, base, siteUrl)}
---
`);
