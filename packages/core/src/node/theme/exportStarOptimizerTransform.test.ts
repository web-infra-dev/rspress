...
`);
  ...
  it('resolves export * through nested modules', async () => {
    const { code, filePath } = await loadFixture('complex');
    const result = await exportStarOptimizerTransform(code, filePath);
  ...
  it('resolves async fn', async () => {
    const { code, filePath } = await loadFixture('async-fn');
    const result = await exportStarOptimizerTransform(code, filePath);
