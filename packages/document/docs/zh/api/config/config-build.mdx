...
- 示例：使用 [output.distPath](https://rsbuild.dev/config/output/dist-path) 将输出目录修改为 `doc_dist`：
...
你也可以覆盖内置的 [@rsbuild/plugin-react](https://rsbuild.dev/zh/plugins/list/plugin-react)、[@rsbuild/plugin-sass](https://rsbuild.dev/zh/plugins/list/plugin-sass) 和 [@rsbuild/plugin-less](https://rsbuild.dev/zh/plugins/list/plugin-less) 插件，并传入相关插件选项。
...
- 默认配置别名的语言包括 `js`、`jsx`、`ts`、`tsx`、`xml`、`md`、`mdx`。
...
每项语言的别名以 `[string, string]` 格式配置，前者为语言的别名，后者为语言的全名，你可以前往[文件列表](https://github.com/react-syntax-highlighter/react-syntax-highlighter/tree/master/src/languages/prism)查看所有支持的语言全名。
