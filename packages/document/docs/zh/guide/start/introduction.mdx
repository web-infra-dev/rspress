...
为了突破传统前端工具的编译性能瓶颈，Rspress 在 Rust 工具链上做了很多的努力，相比与现有的 SSG 框架有明显的性能优势，具体测试数据如下：
...
除了打包工具本身，对于文档框架中另一大编译性能瓶颈，即`Markdown 编译`，我们也将这一流程 Rust 化以进一步提速，基于开源的 [mdxjs-rs](https://github.com/wooorm/mdxjs-rs) 库扩展一系列插件，并通过 [napi-rs](https://napi.rs/) 进一步封装出 Node.js 的调用接口，从而定制出 Rspress 的 Markdown 编译器（即[`@rspress/mdx-rs`](https://github.com/web-infra-dev/mdx-rs)）。这款编译器相比传统的 JS 版本的编译器，有 10 倍以上的性能提升：
