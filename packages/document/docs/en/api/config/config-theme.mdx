...
## redirectToCloestLocale

- Type: `'first-visit' | 'never'`
- Default: `'first-visit'`

Whether to redirect to the closest locale when the user visits the site, the default is `first-visit`, which means that the user will be redirected on the first visit. If you set it to `never`, the user will not be redirected. For example:
