...
1. Rspress has better build performance based on the Rust front-end toolchain. For details, refer to [Build Performance](#build-performance).

::: tip

The Rspack team and the Docusaurus team have cooperated. Docusaurus modernized their infrastructure to use faster Rust-based tools for build performance optimization in [Version 3.6](https://docusaurus.io/blog/releases/3.6#docusaurus-faster) and you can enable the ability to build with Rspack with configs.
