...
In `rspress.config.ts`, you can edit HTML's metadata (aka header tag) on a page basis. We explain it with more details in [basic config](/api/config/config-frontmatter#head) within the api section.
