...
`fixed-with-per-comp` will add an independent html file to each code block based on the `fixed` mode. When clicking on the code block, the right iframe will be switched to display only a single component.
