...
If you have set `defaultInternalBehavior` to `'pure'`, Rspress will not render this component by default, but instead display it as a code block. If you want to render a code block as a component in this situation, you can do add the `preview` identifier to specify, the usage is as follows:
...
### defaultInternalBehavior

The default behavior of rendering internal code blocks that do not have the `pure` or `playground` identifiers declared by users, defaulting to `playground`.
