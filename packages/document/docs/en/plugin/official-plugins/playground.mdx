...
Otherwise, You can use `direction` parameter to specify the layout, `horizontal` and `vertical` is supported:
...
Same as internal components, external components aslo support `direction` attribute:
...
In a custom playground, you can directly import the original editor and renderer, and import pre-packaged dependencies through `playground-imports`:
```ts
...
### include
By default, the plug-in will automatically scan all import statements in the demos, the packages not used in the demos cannot be used in the playground. If you want to add other packages to the playground, you can use the `include` parameter:
