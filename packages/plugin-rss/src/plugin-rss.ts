...
function getRssItems(
  ...
  return Promise.all(
    feeds
      ...
      .map(async options => {
        const after = options.item || ((_: any, feed: FeedItem) => feed);
        const item = await after(page, generateFeedItem(page, siteUrl));
  ...
  return {
    ...
    async extendPageData(_pageData) {
      ...
      const showRssList = new Set(
        concatArray(pageData.frontmatter['show-rss'] as string[] | string),
      );
