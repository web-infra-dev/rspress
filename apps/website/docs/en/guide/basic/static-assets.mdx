...
Rspress will automatically find the image and respond to the browser according to the `.mdx` file path and image path.

On the other hand, static resources can also be imported using absolute paths. In this way, Rspress will look for resources in the `public` folder under the root directory of the project, which is the directory specified by the `root` field in `rspress.config.ts`.
